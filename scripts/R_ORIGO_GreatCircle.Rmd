...
origo$origo_frequency <- origo %>% 
  left_join(orig_frequency, by = "origo_clean") %>% 
  pull(n)
...
# View Province of findspot

origo %>%

  group_by(start_long,start_lat, province_label_clean) %>%
...
Province of origin
Pleiades roman empire is too complicated beause it collates all periods and changing province names. I need a simpler adminitrative overview.
...
### Spatial join with the Roman Empire

 

Alas the Pleiades provinces overlap with different temporal versions of themselves. I need only one dataset, ideally the largest AD117 version

If you need better provinces, download them from DAWE with bash after installing wget tool with https://builtvisible.com/download-your-website-with-wget/
...
## Figuring out birth and death provinces
