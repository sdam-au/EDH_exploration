...
```{r}
# Pleiades provinces
provincePle <- read_sf("../data/pleiades_regions.geojson")
...
origo_pr <- st_read_parquet("../data/origo_geo_provinces.parquet")
```
...
## THis chunk is superceded by origo parquet, but is good just for reproducibility
...
geocoded_m <- read_sheet("https://docs.google.com/spreadsheets/d/17ibTaRJHHevY2wVsx9aK-NEd54fGho69l5dGJNE3XBA/edit#gid=1089791340", range = "2023WORKSHEET")
...
leaflet() %>%
  ...
  # Overlay groups
  addPolygons(data=provincePle, group = "Pleiades provinces") %>% 
...
BDnetwork <- ggplot() +
   ...
   geom_sf(data = provincePle, size = .2, fill = "grey", col = "grey") +
...
ggplot(orig_dest_lines)+
  ...
  geom_sf(data=provincePle, alpha = 0.3)
...
unique(provincePle$name)  # in Pleiades also 120
b_province <- as.data.frame(x = unique(provincePle$name))   
