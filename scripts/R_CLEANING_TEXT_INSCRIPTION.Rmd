...
## 2. Make the request and download the dataset from Sciencedata
```{r, echo = TRUE }
resp = request("EDH_attrs_cleaned_2020-09-17.json", path="/sharingin/648597@au.dk/SDAM_root/SDAM_data/EDH/", method="GET", cred=c(mycred_secret[1], mycred_secret[2]))
...
```{r}
EDH_clean <- tibble::as_tibble(jsonlite::fromJSON(resp))
head(EDH_clean)
```
...
write(EDH_cleaned_json, file="EDH_text_cleaned_2020-09-18.json")
request("EDH_text_cleaned_2020-09-18.json", path="/sharingout/648597@au.dk/SDAM_root/SDAM_data/EDH/",
...
```{r}
file.remove("./EDH_text_cleaned_2020-09-18.json")
remove(mycred_secret)
```
...
# do not run
 texts %>% 
   ...
   unnest_wider(col=text_edition) %>% 
   unnest_wider(col=transcription) %>% 
