...
This script contains faceted maps of origin and destination of the people in EDH inscriptions (93,000 > 3000 placenames> 2277 successfully geocoded ones).
...
*origo dataset that contains both origo and findspot locations converted into numeric coordinates, the former as pleiades_Longitude/Latitude and origo_point columns, the latter as edh_point. 
*LIST with inscription data containing findspot geometry, ancient and modern names as well as other descriptive metadata
*Roman province boundaries from Pleiades and EDH respectively. Pleiades boundaries collate all changes over time, without providing attributes by which one could filter. EDH has invalid geometries, that Vojtech fixed in Python.
...
# how many are manually vs automatically geocoded?
origo %>% 
  st_drop_geometry() %>% 
  filter(!is.na(pleiades_title_list)) #675 auto
  filter(is.na(pleiades_title_list)) #1602 manual
...
# LIRE from https://zenodo.org/record/8147298 (July 2023)
LIRE <- st_read_parquet("../data/large_data/LIRE_v2-3.parquet")
# LIST from https://zenodo.org/record/7993915 (July 2023)
LIST <- st_read_parquet("../data/large_data/LIST_v0-6.parquet") # newest version with 2021 people as a list column of tibbles
...
colnames(LIREfindspot)

# Dataset of vertices in arc that connects origo with findspot, produced by Vojtech for animation
# paths <- st_read_parquet("../data/origo_linepoints_gdf.parquet") # not relevant here
```
...
## Death locations
Death locations are represented by findspot in edh_point attribute. BUT WE ARE MISSING FIND SPOT NAMES in origo, so must grab them from LIRE- or LISTfindspot !! See spatial streamlining below
```{r}
colnames(origo)
...
Create two point simple features using the point columns in origo. Currently the edh-point, the findspot, is the active geometry, but that can be changed. 
Remove 'X' from column names marking presence/absence of person in a given century and project simple features into 3035 for distance caluclations (it's a 2D projection).

```{r spatialize death-and-birthplaces}
# Column names in origo data
names(origo) # edh point is the active geometry
names(origo)[14:19] <- c("1BCE", "1CE", "2CE", "3CE", "4CE", "5CE")

# Deathplace project
deathplace <- origo %>%  # ensure that 
  st_as_sf() %>% 
  st_set_crs(4326) %>% #mapview(zcol = "certainty")
  #st_join(LISTfindspot) %>% 
  st_transform(3035) %>% 
  # add findspot names to the spatial coordinates in deathplace
  full_join(st_drop_geometry(LIREfindspot), by = c("hd_nr"="EDH.ID")) 

# Birthplace
birthplace <- origo %>%  # switching active geometry to the place of origin
  st_drop_geometry() %>% 
...
### Check EDH province and b-d-place alignment: projection and validity
```{r}
...
```{r pivot-long}
# See the ~1BCE fields in the dataset which indicate presence/absence in a given century
names(origo)
...
deathplace_long <- deathplace %>%  
  select(-origo_line) %>% 
  pivot_longer(cols = `1BCE`:`5CE`,  # take values from columns and write through True/False
               names_to = "century",
...
glimpse(deathplace_cntime)
class(deathplace_cntime$n)
deathplace_cntime$cat <- cut(deathplace_cntime$n,
  breaks = c(0, 10, 100, 200),
  labels = FALSE
)

deathtime_map <- 
  tm_shape(deathplace_cntime) +
  tm_facets(by = "century",
           ncol=2, nrow = 3
            )+
  tm_dots(size = "n",
         #style = "equal",
         # style = "fixed", breaks=c(1,10,50,100,200)
          #style = "log10"
          #style = "jenks"
       #  legend.format = list(fun = function(x) {c(0, 10, 100, 200)[x]})
          ) +
...
tmap_save(birthtime_map, "../figures/birthplacemap_scaled.png", width = 15, height = 21, units = "cm")
...
## Deathplace and death province stats
...
### Birthplace and Deathplace provinces - save data
```{r}
...
## Mapview source and attractor provinces
...
## TMAP source and attractor provinces
tmap_mode(mode = "plot" )


b_prov <- tm_shape(provinceEDH) +
  tm_polygons(col ="birth_sum",colorNA = "white", title = "Migrant births")
  
d_prov <- tm_shape(provinceEDH)+
  tm_polygons("death_sum",colorNA = "white", title = "Migrant deaths")

tmap_arrange(b_prov,d_prov, ncol = 2)

# Print as png

png("../figures/Figure07_400dpi.png",
     width= 20, height = 8, units = "cm", res = 400)
tmap_arrange(b_prov,d_prov, ncol = 2)
dev.off()

pdf("../figures/Figure07.pdf",
    #colormodel = "gray", 
    width= 12, height = 5)
tmap_arrange(b_prov,d_prov, ncol = 2)
dev.off()
...
# Fix century names
names(lines)[13:18] <- c("1BCE", "1CE", "2CE", "3CE", "4CE", "5CE")
...
lines %>% 
  ...
  ggplot(aes(x = length_km,fill = certainty))+
  geom_histogram(binwidth = 100) +
  ggtitle(label = "Birth to death distances by geocoding certainty")+
  scale_fill_grey()+
  theme_bw()+
  ...
  labs(x = "Distance (km)",
       y = "Persons",
       fill = "Origo geocoding")+
  geom_vline(aes(xintercept = mean(length_km)), linetype = "dashed", size = 1)

ggsave("../figures/orig_dist_certain_bw.png", width = 6, height = 4)
```
...
### Lengthening lines for time-series
```{r long-lines}
...
ggsave("../figures/orig_distc_centuries_bw.png", width = 20, height = 15, units = 
        "cm")
...
### Origo by destination province : where do people come from to province
individual province origo
```{r province-mapview}

top_d_prov <- death_smr %>% 
  top_n(6) %>%
  pull(province)

# Check attraction provinces
lines_long %>% 
 # filter(province == "Moesia superior") %>% 
  # filter(province == "Dalmatia") %>% 
   filter(province == "Dacia") %>% 
  mapview(zcol = "century") + mapview(provinceEDH)
 #,"Dalmatia", "Moesia superior", "Pannonia superior","Baetica","Pannonia inferior"  
  

# select 6 most popular destination province to lines
lines_6province <- lines_long %>% 
  #filter(province%in%top_d_prov) %>% 
  filter(province %in% c("Germania superior","Dalmatia", "Moesia superior"
                         ,"Pannonia superior","Baetica","Pannonia inferior"))  
  
lines_6province %>% 
  #distinct(province_label_clean)
  mapview( zcol = "province")


```

TMAP facetted province origo is NOT WORKING CORRECTLY HERE!! US EMAPVIEW INSTEAD
```{r}
# Facetting the map TMAPS does not match mapview(). something is wrong.
library(tmap)
tmap_options(limits = c(facets.view = 6)) # we want to view 5 periods
tmap_options(check.and.fix = TRUE)
tmap_mode(mode = "view" )

tm_shape(lines_6province) +
  tm_facets(by = "province",
         
            ncol=2, nrow = 3)+
  tm_lines()+ #"century",       palette = "magma"
  tm_shape(provinceEDH)+
  tm_borders(col = "grey",
             alpha = 0.7)+
  tm_layout(main.title = "Final destination of movement",
...
# Sorting to see where I gained 11 people? (from 2277)
...
# Explore people and B-d-distances
lines_people %>% 
  st_drop_geometry() %>% 
  select(hd_nr, pleiades_title, findspot_ancient_clean, length_km) %>% 
  filter(length_km<50) %>% 
  arrange(length_km)
```

Beware that some 139 persons have died in the same region as they were born - within 10kms anyway, and 217 within 50kms.
...
ggplot(lines_people, aes(x = length_km))+
  ...
  ggtitle(label = "Distances from birth region to death region")+
  theme_bw()+
  labs(x = "Distance in kms",
       y = "Number of travelers")
...
```{r gender-dist}
# Summaries of distance by gender
...
lines_people %>% 
  st_drop_geometry() %>% 
  #group_by(gender) %>% 
  ggplot(aes(x= gender, y = length_km)) +
  geom_violin(aes(fill = gender), alpha = 0.7,
              show.legend = FALSE) +
  scale_fill_manual(values = c("red", "blue", "white")) +
  geom_jitter(aes(fill = gender), shape = 21,
             position = position_jitterdodge(dodge.width = 0), 
             alpha = 0.5, 
             show.legend = FALSE) +
  scale_fill_manual(values = c("red", "blue", "white")) +
  theme_bw()+
  labs(y = "Distance migrated (kms)",
       x = "")
...
lines_people %>% 
  filter(!is.na(gender)) %>% 
  ggplot(aes(x = length_km, fill = gender))+
  geom_histogram(binwidth = 100) +
  scale_fill_grey()+
  ggtitle(label = "Birth to death distances by gender")+
  theme_bw()+
  theme(legend.title=element_blank(),
        legend.position = c(.85,.85))+  # works only after theme_bw()
  labs(x = "Distance (km)",
       y = "Gendered travelers")

ggsave("../figures/orig_dist_gender_bw.png", width = 6, height = 4)
```
### Trip length by gender

```{r women-dist}
# Breakdown of women distance
lines_people %>% 
  filter(gender == "female") %>% 
  pull(length_km) %>% summary()

#  Long distance travel female
lines_people %>% 
  filter(gender == "female") %>%
  filter(length_km > 1000)
  pull(length_km) %>% summary()
...
lines_people %>% 
  filter(gender == "female") %>% 
  ggplot(aes(x = length_km))+
  geom_histogram(bins = 100, aes(fill = gender), show.legend = FALSE)+
  ggtitle(label = "Birth to death distances by women")+
  theme_bw()+
  # theme(legend.title=element_blank(),
...
lines_people %>% 
  filter(gender == "male") %>% 
  ggplot(aes(x = length_km))+
  geom_histogram(bins = 100, fill = "blue", color = "blue", show.legend = FALSE)+
  ggtitle(label = "Birth to death distances by men")+
  theme_bw()+
  # theme(legend.title=element_blank(),
  #       legend.position = c(.85,.75))+  # works only after theme_bw()
  labs(x = "Distance in kms",
...
status <- str_split_fixed(lines_people$status.y, ";", n=3) %>% 
  as.data.frame()
...
```{r}
# Plot status by distance
ggplot(data = lines_status,  aes(x = length_km))+ # I removed fill = certainty
  geom_histogram(binwidth = 100)+
  facet_wrap(~ status_clean)+
  theme_bw()+
  theme(legend.position = "bottom",
         text = element_text(size = 14))+
  labs(title = "Birth to death distances by migrant status (1477 out of 2277)",
       #fill = "Origo location" , # certainty is removed
       x = "Distance (km)",
        y = "Persons" )

ggsave("../figures/orig_dist_occupfac.png", width = 10, height = 8)
```
...
## Checking the oddities and deeper explorations
...
In a single inscription from Viminacium (near modern Kostolac), over 40 individuals (soldiers and others) are mentioned in a list below dedication. So HD032316 should allocate 40 people to Moesia Superior as a deathplace, and maybe also birthplace (many are from Scu    pi and Remesiana, others from Salona, Montana and others). All are military with various roles mentioned, such as interpreter, guardian of arms (custos armorum), signifer, imaginifer, etc.
...
## Rome
```{r lazio-Rome}
# How many inscriptions in LIRE were found at  Rome? 
LIRE %>% 
  filter(findspot_ancient_clean == "Roma") # 3651

LIST %>% 
  filter(findspot_ancient_clean == "Roma") # 4357

LIRE %>% 
  group_by(findspot_ancient_clean) %>% 
  tally() %>% arrange(desc(n)) # 3651 

LIST %>%
  st_drop_geometry() %>% 
  group_by(findspot_ancient_clean) %>% 
  tally() %>% arrange(desc(n)) # 4357 

deathplace %>%
  st_drop_geometry() %>% 
  group_by(province) %>%  # check for most common attractor province
  tally() %>% arrange(desc(n)) 

deathplace %>%
  st_drop_geometry() %>% 
  #group_by(province) %>%  # check for most common attractor province
  group_by(findspot_ancient_clean) %>% # province and city
  tally() %>% arrange(desc(n)) 
...
status_rome %>% 
  ...
  count(combined_clean, sort=TRUE) -> status_rome_counts

```

```{r}
# What is the situation with Rome and Lazio et Histria being not so frequent birth or deathplace?
birthplace_prov %>% 
   distinct(province) %>% 
   pull(province) %>% 
   sort()
...
## Searching for Statius Priscus and Severus 
```{r}
# Search for M Statius Priscus, a famous general who crisscrossed the RE
people_or_unnested %>% 
  filter(nomen == "Statius")
  filter(cognomen== "Priscus")
  # no result, not in origo dataset
  
# Search in EDH people (R_ORIGO.Rmd)  
Priscus <- EDH_people%>% 
  filter(nomen == "Statius") %>% 
  filter(cognomen== "Priscus") %>% 
  pull(id)
# 4 inscriptions mention him, but no origo is listed

# Findspots for M Statius Priscus
LIST %>% 
  st_set_crs(4326) %>% 
  filter(EDH.ID%in%Priscus) %>% 
  mapview() + mapview(provinceEDH)
```

###  Sextus Julius Severus
Is this famous general who pacified Dacia, Britannia and Armenia (fromt eh Parthians) findable in our inscriptions?
Born in Luceria in Apulia, is his origo in the EDH/LIRE?

```{r}
# Search for
people_or_unnested %>% 
  filter(nomen == "Iulius") %>%   # 108
  filter(cognomen== "Severus") # 37 if only Severus is queried
  # only Quintus Iulius Severus DInia found. Sextus not in origo dataset
  
# Search in EDH people (R_ORIGO.Rmd)  
ISeverus <- EDH_people%>% 
  filter(nomen == "Iulius") %>%   # 1384
  filter(cognomen== "Severus") %>% # 14 individuals match
  filter(status == "senatorial order") %>%  # 2 individuals but no origo! 
  pull(id)
# 2 inscriptions with Iulius Severus , but no origo is listed and not sure this is him

# Findspots for Sextus Julius Severus
LIST %>% 
  st_set_crs(4326) %>% 
  filter(EDH.ID%in%ISeverus) %>% 
  mapview() + mapview(provinceEDH)
```

### All findspots versus origo findspots
```{r}
# Number of unique textual findspot placenames in LIST
length(unique(LIST$findspot_ancient_clean))  # 2242

# Number of coarse textual findspot placename variants in LIST
length(unique(LIST$findspot_clean)) # 20053

# Number of unique geocoded placenames in LIST
length(unique(LIST$Latitude)) #21172
length(unique(LIST$Longitude)) #21198
length(unique(LIST$geometry)) # 21277
