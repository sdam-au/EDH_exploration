...
## Death locations
Death locations are represented by findspot in edh_point attribute.
```{r}
colnames(origo)
...
knitr::kable(total_travellers, col.names = c("Century","1BCE", "1CE", "2CE", "3CE", "4CE", "5CE"),  align = "lcccccc", caption = "Traveler volume by century.") %>% 
    kable_styling(latex_options = "striped")


```

# Facet distances by century 
```{r}

```


## Birth and death province statistics
Let's create some summaries of most frequent birth and death places
```{r b-d-summaries}
## Deathplace and Birthplace stats
death_sum <- deathplace %>% 
...
## Birth and death provinces' names in origo
Here we attach birth and death province names to the origo data and generate summaries of province flow.
```{r b-d-prov-origo}
...
```
## Break distances down by century
...
# Histogram of distances
ggplot(lines_long %>% filter(certainty == "certain"), aes(x = length_km))+
  geom_histogram(bins = 100, aes(fill = certainty) )+
...
# Histogram of distances
ggplot(lines_long, aes(x = length_km))+
  geom_histogram(bins = 100, aes(fill = certainty) )+
  ggtitle(label = "Life to death distance over centuries") +
  theme_bw() +
  theme(#legend.title=element_blank(),
...
Try with LIRE people
```{r eval=FALSE}
# Use LIRE
head(LIRE$EDH.ID)
LIRE <- LIRE %>% 
  dplyr::filter(EDH.ID %in% origo$hd_nr)
class(LIRE$people)
LIRE$people[1:2]
```
...
```{r eval=FALSE}
ggplot() +
  stat_density_2d(data = birthplace_long,
                  mapping = aes(x = purrr::map_dbl(birthplace, ~.[1]),
                                y = purrr::map_dbl(birthplace, ~.[2]),
                                fill = stat(density)),
                  geom = "tile",
                  contour = F,
                  alpha = 0.5)+
