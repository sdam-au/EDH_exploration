...
EDH_tibble<- EDH_tibble %>% 
  mutate(dis_manibus_in_context40 = str_extract(EDH_tibble$clean_text_interpretive_word_lowercase, "(\\w*\\s){1,6}\\bd m s\\b|\\bd m\\b|dis manibus sacrum|dis manibus(\\w*\\s){1,6}")) 
```
...
jsonlite::write_json(EDH_tibble_json, path="../data/EDH_dis_manibus_2021-08-12.json")

sdam::request("../data/EDH_dis_manibus_2021-08-12.json", path="/sharingout/648597@au.dk/SDAM_root/SDAM_data/EDH/public", force = TRUE,
...
DM_map<- head(EDH_DM, 100) %>% 
  ...
  addCircles(lng = EDH_dis_manibus$longitude, 
             lat = EDH_dis_manibus$latitude, opacity = 0.1, radius = 1, fill = T , color = "blue" , fillColor = "blue",) %>%
  addCircles(lng = EDH_dis_manibus_sacrum$longitude, 
             lat = EDH_dis_manibus_sacrum$latitude, opacity = 0.1, radius = 1, fill = T , color = "red" , fillColor = "red",) %>%
  addCircles(lng = EDH_d_m_s$longitude, 
             lat = EDH_d_m_s$latitude, opacity = 0.1, radius = 1, fill = T , color = "yellow" , fillColor = "yellow",)  %>% 
  addCircles(lng = EDH_d_m$longitude, 
             lat = EDH_d_m$latitude, opacity = 0.1, radius = 1, fill = T , color = "orange" , fillColor = "orange",) %>% 

  
  #addAwesomeMarkers(~EDH_tibble$longitude, ~EDH_tibble$latitude, icon=icons) %>% 
  addLegend(position = "bottomright",
  colors = c("Red", "Blue", "Orange", "Yellow"),
  labels = c("dis manibus sacrum", "dis manibus", "d m", "d m s"), opacity = 1,
  title = "Dis Manibus formula on inscriptions (EDH)" 
) %>% 
...
# smaller dataset
EDH100<- head(EDH_DM, 100)

# first 50 characters of the text of an inscription
EDH100<- EDH100 %>% 
  mutate(text50 = substr(EDH100$clean_text_interpretive_word_lowercase, 1,50))

# Damerau-Levenshtein method - allows for substitution, deletion, insertion and transposition of individual characters

dl_distance<- stringdistmatrix(EDH100$dis_manibus_in_context40, EDH100$dis_manibus_in_context40, method= "dl")
table(dl_distance)
...
dfClust <- data.frame(idnames, cutree(hc, k=3))
...
left_join(EDH100, dfClust[1:2], by=c("id"="id"))
```
...
library("stringdist")
m <- stringdistmatrix(rawnames, rawnames, method = "dl")
m
rownames(m) <- rawnames
colnames(m) <- rawnames
m[!upper.tri(m)] <- NA
