---
title: "Use of Eagle vocabularies in the EDH dataset"
...
3. Display the first 6 records of the full dataset
```{r, display head}
head(EDH_tibble)
```
...
Epigraphic Database Heidelber (https://edh-www.adw.uni-heidelberg.de/) uses linked open data (LOD) from EAGLE Project vocabularies (https://www.eagle-network.eu/resources/vocabularies/) in the following five Epidoc XML tags:
...
The aim of this script is to analyse the usage of EAGLE vocabularies in the EDH as part of the Epigraphy.info working group tasked with cleaning up the EAGLE vocabularies.
...
Total 346 record are missing information extracted from XML files (due to delay in availability of published data through EDH data dumps on their website). However, at least partial information about them can be extracted from the EDH API, but for the purpose of the current evaluation of the Eagle vocabularies are these 346 records considered as missing as we cannot access their respective XML files with the vocabularies.
```{r}
EDH_tibble %>% 
...
mycred_secret<- readLines("~/mysecret.txt")
write(EDH_missingXML_json, file="EDH_missingXML_20201110.json")
  request("EDH_missingXML_20201110.json", path="/sharingout/648597@au.dk/SDAM_root/SDAM_data/EDH/public",
        method="PUT", cred=c(mycred_secret[1], mycred_secret[2]))
remove(mycred_secret)
```

## Total number of Eagle vocabularies in EDH dataset
```{r}
#object
EDH_eagle %>% 
...
#how many eagle LOD has been used in the EDH
