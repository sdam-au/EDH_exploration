...
```
There are 15 columns with spatial/geographic information in the people attribute. Except for origo, they originate from the inscription itself, describing its provenance or context of discovery.
...
EDH_people %>% 
  ...
  mutate(ratio = round(cnt/sum(cnt)*100, 1))
```
Women travel less or declare their origo less, as the normal ratio in EDH is ca 15:65 women to men, while in the corpus with origo it is a whooping 5:85 (with the residuals being NA gender values)

## What kind of inscription has origo
```{r}
EDH_people %>%  # was EDH_full
  filter(!is.na(origo)) %>% 
...
Origo features mostly in epitaphs followed by honorific and votive inscriptions, lists and military diplomas. Epitaphs are personal, while most others can be also more public.

## What status did the bearers have?
Some people declare multiple so this is the maximum boradest picture.
```{r}
...
status_counts %>% 
  ...
  labs(x = "Number of instances (log10)", y = "Status category", title = "Overview of status references in the EDH dataset", subtitle = ggtitle(paste("n =", nrow(has_origo), "inscriptions"))) +
...
## Bring in data geocoded by Vojtech 
in py_geocode_origo
...
## Connect Birth-Death places 
Great circles in geodetic space
...
## Walk around the Empire with MapBoxAPI
