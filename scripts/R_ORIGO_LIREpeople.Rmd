...
status_one <- status %>% 
  ...
  group_by(status) %>% 
  count(status, sort = TRUE) #%>% arrange(desc(n))
...
### log version
```{r}
# with multiple statuses all collated 1277 soldiers
status_counts %>% 
  mutate(log=round(log10(n),1)) %>% 
  mutate(combined_clean = reorder(combined_clean, log)) %>% 
  ggplot(aes(y=combined_clean, x=log, fill=combined_clean)) +
  ...
  labs(x = "Number of instances (log10)",
       y = "Status category",
       title = "Status references in the origo dataset",
    subtitle = ggtitle(paste("n =", nrow(status), "people in inscriptions"))) +
  geom_label(aes(label= n)) +
  theme_linedraw(base_size = 13) +
  theme(legend.position = "none")

# single status per person (last, highest) 1264 soldiers
status_one %>% 
  mutate(log =round(log10(n), 1)) %>% 
  ...
  labs(x = "Number of instances (log10)", 
       y = "Status category", 
       title = "Status references in the origo dataset", 
       subtitle = ggtitle(paste("n =", nrow(origo_people), "people in inscriptions"))) +
  geom_label(aes(label= n)) +
  theme_linedraw(base_size = 13) +
  theme(legend.position = "none")
ggsave("../figures/Status_overviewlog.png", width = 8, height = 6) 
