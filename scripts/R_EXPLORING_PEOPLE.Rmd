...
status_counts %>% 
  ...
  ggplot(aes(y=combined_clean, x=n), size = n) +
  geom_col(fill= "magenta") +
  labs(x = "Number of instances", y = "Status category", title = "Overview of status references in the EDH dataset", subtitle = "n = 81,476 inscriptions" ) +
...
ages$test<- ifelse(filter(ages$ages_years == 0), ages$total_age, NA)
...
age_provinces %>% 
  ...
  labs(y="Roman province", x="Years", title= "Average age of people from inscriptions in the EDH database by province", subtitle = "n= 7993 people")+ 
...
epitaph %>% 
  ...
  geom_col(color="white", fill="orange", width=0.8, stat="identity") +
...
epitaph %>% 
  ...
  geom_label(aes(label= ratio_total)) +
  geom_text(aes(label = ratio_total), colour = "red", fontface = "bold", vjust = -1) %>% 
  print()
