...
```{r}
list_json <- jsonlite::fromJSON("https://sciencedata.dk/public/b6b6afdb969d378b70929e86e58ad975/EDH_text_cleaned_2021-01-21.json") # this dataset still works

list_json2 <- jsonlite::fromJSON("https://sciencedata.dk/public/b6b6afdb969d378b70929e86e58ad975/EDH_text_cleaned_2023-04-26.json") # is not working as the dataset lacks the person_id in separate column
...
```{r}
list_json <- jsonlite::fromJSON("../data/large_data/EDH_text_cleaned_2021-01-21.json") #rror in parse_con(txt, bigint_as_char) : parse error: premature EOF
list_json <- jsonlite::fromJSON("../data/large_data/EDH_text_cleaned_2022-11-03.json") 
EDH_tibble <- as_tibble(list_json2) # as_tibble() is boxing columns up into lists
#dir.create("../figures")
```
...
People attribute contains many more attributes 
```{r}
EDH_tibble$people[1:2]
```

```{r}
# unnesting the nested dataframe people
EDH_unnested<- EDH_tibble %>% 
  dplyr::select(id, people) %>% 
  unpack(people)  %>% 
  unnest_wider(people, simplify = TRUE) 

#Error in col_to_wide(col = data[[col]], name = col_name, strict = strict, :
...
# saveRDS(EDH_people,"../data/large_data/edh_people21.rds")
