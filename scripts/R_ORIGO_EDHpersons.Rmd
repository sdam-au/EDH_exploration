...
# Extract non-missing people from LIRE (ERRORS OUT ON MY COMP)
...
## Working through LIST findspots

Working with findspot from the LIST
...
# Isthm and 
  LIST %>% 
    ...
    select(EDH.ID, findspot_clean, findspot_ancient_clean)
```
There are additional 14 columns with spatial/geographic information in the enriched LIST attributes. Except for origo (which is embedded in people), they originate from the inscription itself, describing its provenance or context of discovery.


# Workgin with ORIGO  in EDH_person_data.csv
```{r}
...
EDH_persons %>% 
  filter(hd_nr == "HD069852")
```

The answer is 2672 people out of 93646, which is barely 3% , or precisely: `2672/93646*100` %
These 2672 people originate from 1903 distinct places. For these we can pin a point in the map for 676 locations or 36%.
...
EDH_persons %>% 
  select(origo, geschlecht) %>% 
  filter(!is.na(origo)) %>% 
  group_by(geschlecht) %>%
  summarize(cnt = n()) %>% 
  mutate(ratio = round(cnt/sum(cnt)*100, 1))
```
Declarations of origo are less frequent among women then men. The ratio in EDH is ca 15:65 women to men, while in the corpus with origo the ratio is even more skewed with only 6% of women and 85% of men (with the remaining 10-20% being NA values)

## What kind of inscription has origo
To explore other inscription-derived attributes of people iwth origin, we need to use LIST_persons - a joined dataset of origo and EDH attributes
...
```{r}
# Link persons with LIST inscription attributes
LIST_persons <- EDH_persons %>% 
  select(hd_nr, id, geschlecht, origo, status) %>% 
  left_join(LIST, by = c("hd_nr" = "EDH.ID"))

LIST_persons %>%  # linked persons with LIST
  filter(!is.na(origo)) %>% 
  count(type_of_inscription_clean, sort=T) %>% 
  mutate(ratio = round(n/(sum(n)/100),2))

LIST_persons %>%  # was LIST_persons
  filter(!is.na(origo)) %>% 
  count(type_of_inscription_clean, sort=T) %>% 
  mutate(ratio = round(n/(sum(n)/100),2)) %>% 
  ...
  labs(fill = paste0("Inscription Type (n = 2672)"))+
...
```{r}
# Check the contents of the 'status' column in the LIST_persons
colnames(LIST_persons)
LIST_persons$status.y

# Subset to inscriptions with origo
has_origo <- LIST_persons %>% 
  filter(!is.na(origo))
...
```{r}
str_split_fixed(EDH_persons$status, ";", n=3) %>% 
...
status_counts %>% 
  ...
  labs(x = "Number of instances (log10)", y = "Status category", title = "Status references in the EDH dataset", subtitle = ggtitle(paste("n =", nrow(EDH_persons), "people in inscriptions"))) +
  geom_label(aes(label= n)) +
  theme_linedraw(base_size = 13) +
  theme(legend.position = "none")
...
EDH_persons %>% 
  ...
  filter(!is.na(EDH_persons$`age: years`) | !is.na(EDH_persons$`age: months`) | !is.na(EDH_persons$`age: days`) |!is.na(EDH_persons$`age: hours`))
```


## What are the unique values for years
```{r}
unique(EDH_persons$`age: years`)
```

## How many people have their age stated in years
```{r}
sum(!is.na(EDH_persons$`age: years`))
EDH_persons %>% 
  dplyr::select('age: days', 'age: months', 'age: hours', 'age: years') %>%
  filter(!is.na(EDH_persons$`age: years`))
```

## How many people have their age stated in months
```{r}
sum(!is.na(EDH_persons$`age: months`))
EDH_persons %>% 
  dplyr::select('age: days', 'age: months', 'age: hours', 'age: years') %>%
  filter(!is.na(EDH_persons$`age: months`))
```

## How many people have their age stated in days
```{r}
sum(!is.na(EDH_persons$`age: days`))
EDH_persons %>% 
  dplyr::select('age: days', 'age: months', 'age: hours', 'age: years') %>%
  filter(!is.na(EDH_persons$`age: days`))
```

## How many people have their age stated in hours
```{r}
sum(!is.na(EDH_persons$`age: hours`))
EDH_persons %>% 
  dplyr::select('age: days', 'age: months', 'age: hours', 'age: years') %>%
  filter(!is.na(EDH_persons$`age: hours`))
```
...
```{r}
summary(as.numeric(EDH_persons$`age: years`))
summary(as.numeric(EDH_persons$`age: months`))
summary(as.numeric(EDH_persons$`age: days`))
summary(as.numeric(EDH_persons$`age: hours`))
```
...
EDH_persons %>% 
  dplyr::select('age: days', 'age: months', 'age: hours', 'age: years') %>% 
  mutate(age_years = as.numeric(str_extract(EDH_persons$'age: years', pattern = "[:digit:]+"))) %>% 
  mutate(age_months = as.numeric(str_extract(EDH_persons$'age: months', pattern = "[:digit:]+"))) %>%
  mutate(age_days = as.numeric(str_extract(EDH_persons$'age: days', pattern = "[:digit:]+"))) %>%
  mutate(age_hours = as.numeric(str_extract(EDH_persons$'age: hours', pattern = "[:digit:]+"))) -> ages
```
...
EDH_age<- EDH_persons %>% 
  ...
  mutate(age_months = as.numeric(str_extract(EDH_persons$'age: months', pattern = "[:digit:]+"))) %>%
  mutate(age_days = as.numeric(str_extract(EDH_persons$'age: days', pattern = "[:digit:]+"))) %>%
  mutate(age_hours = as.numeric(str_extract(EDH_persons$'age: hours', pattern = "[:digit:]+"))) %>% 
  mutate(months_to_years = age_months / 12) %>% 
  mutate(days_to_years = age_days / 365) %>% 
  mutate(hours_to_years = age_hours / (24*365)) %>% 
  replace_na(list(months_to_years = 0, days_to_years = 0, hours_to_years = 0)) %>% 
